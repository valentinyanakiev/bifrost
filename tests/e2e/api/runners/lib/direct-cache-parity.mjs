...
// the provider whose numbers are currently in question. Every cell costs four live calls (two
// legs x two rounds) against a ~5.9K-token prefix, so this is a diagnostic instrument, not a
// sweep - widen it when a specific question needs it.
const CELLS = [
...
${shape.hitRate}
// Round 1 starts the series fresh. Collection variables outlive a single iteration, so
// without the reset a second iteration would append to the first one's series and a later
// "best" could be satisfied by a hit recorded in an earlier run.
${
  round === 1
    ? `pm.collectionVariables.set(${J(seriesVarFor(cell.id, leg))}, JSON.stringify([hit]));`
    : `var series = [];
...
// Round 2 reads the write from round 1 of the SAME leg.
function roundTwoScript(cell, leg) {
  const shape = SHAPES[cell.shape];
  const label = `${cell.provider}/${cell.directModel}`;

  const common = `
if (pm.response.code >= 400) {
  pm.test(${J(`Direct-cache-parity [${label}] ${leg} round 2 responded`)}, function () {
...
`.trim();
  ...
  if (leg === "direct") {
    ...
    return `${common}
console.log('[direct-cache-parity] ${cell.id} direct round 2 (read) hit=' + (hit * 100).toFixed(1) + '%');`;
  }
...
// control. A direct leg that errored already failed its own "round 2 responded" assertion.
          ...
          event: [
            ...(isRead ? [{ listen: "prerequest", script: { type: "text/javascript", exec: [SETTLE] } }] : []),
            {
              ...
              script: {
                ...
                exec: (isRead ? roundTwoScript(cell, leg) : roundOneScript(cell, leg, round)).split("\n"),
              },
            },
          ],
